...
qsub_script = open("annotate.sh","w")
qsub_script.write(
"""#!/bin/bash
#SBATCH --job-name=aim3_annotate         # Job name
#SBATCH --partition=batch             # Partition (queue) name
#SBATCH --nodes=1
#SBATCH --ntasks=4                   # Run on a single CPU
#SBATCH --mem=50gb                     # Job memory request
#SBATCH --time=24:00:00               # Time limit hrs:min:sec
#SBATCH --output=aim3_annotate.%j.out    # Standard output log
#SBATCH --error=aim3_annotate.%j.err     # Standard error log
...
cd $SLURM_SUBMIT_DIR
source activate /scratch/noahaus/aim_3/worldwide_data/examp/prokka


prokka --outdir $OUT --cpus 4 --prefix $OUT.annot $ASSEM
cp ./$OUT/$OUT.annot.gff ./
...
assem = sorted(glob.glob('*.scaffold.fasta'))
...
out = list(map(lambda raw: re.sub('.scaffold.fasta','',raw), assem))

#qsub -v reference=/path/to/reference.fa bash.sh
for i in range(len(assem)):
    os.system("sbatch --export=assem={},out={} annotate.sh".format(assem[i],out[i]))
    print(logger("annotation performed on {}".format(assem[i])))

os.remove("annotate.sh")
